...
export default function PengirimanDetailPage() {
  ...
  const buildVariables = () => {
    if (!data) return {}
    const pdfUrl = `${window.location.origin}/api/export/public/pdf/${data.id}`
    return {
          ...
          <div className="relative bg-card border border-border rounded-2xl p-4 sm:p-6 w-full max-w-2xl max-h-[92vh] overflow-y-auto shadow-2xl animate-fade-in space-y-3.5 sm:space-y-4">
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-border">
              <button
                type="button"
                onClick={() => { setWaModal(false); setWaTarget(''); setSelectedContactId('') }}
                className="px-4 py-2.5 border border-border rounded-xl text-xs font-semibold text-foreground hover:bg-secondary"
              >
                Batal
              </button>
              <button
              ...
              >
                ...
                <span>{waSending ? 'Mengirim...' : 'Kirim Sekarang'}</span>
              </button>
            </div>
          </div>
        </div>
